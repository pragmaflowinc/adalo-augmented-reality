...
const MAPS_API_KEY = '{Your_Key}'
const PlacesAPIURL = (lat,lng) => `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=500&key=${MAPS_API_KEY}`;
...
class AdaloARScene extends Component {
  ...
  getNearbyPlaces = async () => {
    ...
    .then((responseJson) => {
      ...
      if(responseJson.status === 'OK'){
        ...
        this.setState({nearbyPlaces: places}, this.placeARObjects);
      }
      else{
  ...
  render() {
    return (
      <ViroARScene onTrackingUpdated={this._onInitialized} >
        ...
        <ViroText text={`${this.state.locationReady}`} scale={[.5, .5, .5]} position={[0, 0, 1]} style={styles.helloWorldTextStyle} />
        <ViroText text={`${this.state.cameraReady}`} scale={[.5, .5, .5]} position={[0, 0, 2]} style={styles.helloWorldTextStyle} />
        <ViroText text={JSON.stringify(this.state.location)} scale={[.5, .5, .5]} position={[0, 0, -2]} style={styles.helloWorldTextStyle} />
        <ViroText text={`${this.state.compassHeading}`} scale={[.5, .5, .5]} position={[0, 1, -1]} style={styles.helloWorldTextStyle} />
